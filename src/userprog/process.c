...
process_execute (const char *file_name)
{
    ...
    /* 스레드 생성 */
    /* 주의: thread.c에서 fd_table을 할당하므로 여기서는 별도 처리가 필요 없습니다. */
...
start_process (void *aux_)
{
    ...
    /* 주의: fd_table은 여기서 할당하지 않습니다. (thread_create에서 이미 할당됨) */
#endif
...
process_exit (void)
{
    ...
    /* 열린 파일 디스크립터(FDT) 정리 */
    /* 모든 열린 파일을 닫아주어야 read-normal 등의 테스트를 통과함 */
...
load (const char *file_name, void (**eip) (void), void **esp)
{
    ...
    *eip = (void (*) (void)) ehdr.e_entry; //프로그램의 진입점 저장
    ...
    if (success) t->bin_file = file; //파일 객체 주소 보관
